...
import { getTumorTypeNameWithExclusionInfo } from '../util/OncoKbUtils';
...
const cancerdrugsUrl = getLocalStorageValue('cancerdrugsUrl');
const cancerdrugsJsonUrl = getLocalStorageValue('cancerdrugsJsonUrl');
        ...
        async function promiseDrugInfo(drug: string): Promise<DrugInfo> {
            try {
                let getResponse = await request.get(
                    cancerdrugsJsonUrl + drug.replace(/\s+/g, '_') + '.json'
                );
            ...
            return {
                name: drug,
                drugInfo:
                    promiseDrugInfos.find(pdi => pdi.name === drug)?.drugInfo ||
                    [],
            };
        }
